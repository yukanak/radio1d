                ...
                for error in '1e-12' '10e-12' '100e-12' '300e-12' '1e-9'; do
                    for correlated in 'True' 'False'; do
                        for ndishes in 32 64 128; do
                            ...
                            bsub -q long -C 0 -W 2:00 -R select[rhel60] -o logfile_${npix}_${redundant}_${sky}_${seed}_${error}_${correlated}_${ndishes}.txt ./plot_random_sky_slices_ps_rmap.py $npix $redundant $sky $seed $error $correlated $ndishes
                        done
                    done
                done
            done
