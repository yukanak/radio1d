...
def plot_sky_slices_stat(npix, redundant, sky, seed, error, correlated, ndishes, path='slac', second_plot='terr'):
    ...
    rmap_no_error = t.get_obs_rmap(uvplane, time_error_sigma=0, filter_FG=False)
    if second_plot == 'terr':
        rmap_with_error = t.get_obs_rmap(uvplane, time_error_sigma=error, correlated=correlated, seed=seed, filter_FG=False)
    ...
    fig = t.plot_rmap_ps_slice(ps_binned_no_error, ps_binned_with_error, k_modes_no_error, alpha_binned_no_error, Nfreqchunks=4, difference_ps_binned=difference_ps_binned)
    if path is not None:
        fig.savefig(path)
        plt.close(fig)
